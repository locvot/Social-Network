...
class UsersService {
  ...
  private signACccessAndRefreshToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
  ...
  async register(payload: RegisterReqBody) {
    ...
    // Server send email to user -> User click link to verify email
    // -> Client send request to server to verify email -> Server verify email -> Client receive access_token, refresh_token
  ...
  async verifyEmail(user_id: string) {
    const [token] = await Promise.all([
      this.signACccessAndRefreshToken({ user_id, verify: UserVerifyStatus.Verified }),
